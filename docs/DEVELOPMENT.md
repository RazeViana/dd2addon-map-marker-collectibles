...
The main entry point is `reframework/autorun/raze_MapMarkersAndCollectables.lua`. Its modules handle settings, nearby selection, minimap rendering, object icons, collectible fog visibility, and sprite-pool expansion. Diagnostics and probes are development tools outside the release runtime.
...
`hide_unexplored_chests`, `hide_unexplored_beetles`, and `hide_unexplored_tokens` each default to false; only a saved boolean true enables them. The three **Hide ... in unexplored areas** controls independently filter all seven chest categories, Golden Trove Beetles, and Seeker's Tokens on both maps, after normal category/acquisition selection. Each marker type's `fog_setting` names its preference. The original chest preference is preserved when updating the earlier chest-only 1.3 package; the new preferences start off. `fog_of_war.lua` reads `GuiManager:getMaskInfo(ui.LocalAreaNow)` and calls the exact `MapMaskInfo:isMaskOff(via.vec3)` overload with each affected collectible's world coordinates. Both UI fields and method signatures were checked against the installed executable's TDB 83 metadata without starting the game; `MaskBit` initialization is checked before querying. No fog setters or writes to the game's saved exploration data are used.

Full-map filtering runs after the marker cache and display-range check on each native setup, so reopening or rebuilding the map reads current fog. Minimap filtering runs each update before distance sorting and the marker limit, while collectible positions can remain cached. Hidden collectibles therefore do not consume available marker slots, and newly revealed collectibles appear without waiting for that cache to expire. Area masks are resolved on every pass and never retained across save or area changes. Missing fog data or a failed lookup hides markers for the selected types and shows a recoverable status message; types with their fog preference off retain normal visibility. On 2026-09-10, Raze confirmed the build with all three fog controls works in-game. That confirmation does not enumerate individual fog-edge or local-area scenarios.
...
The build writes `dist/Map-Markers-and-Collectables-by-Raze-v1.3.zip` and refreshes the matching archive, changelog, file description, checksum, and package manifest in `nexus-upload/1.3/`. The earlier 1.0 and 1.2 packages remain unchanged. Nexus page descriptions use plain text; the current copy is `nexus-upload/1.3/DESCRIPTION.txt`, selected by `description_file` in the listing. Every release must also supply a nonempty `main_file.description` of at most 250 characters in its `listing.json`, as recorded in [AGENTS.md](../AGENTS.md). The builder validates it before modifying release outputs and writes the copy-ready `FILE-DESCRIPTION.txt`. GitHub release notes remain in `nexus-upload/1.3/RELEASE-NOTES.md`. An explicit file list includes only the seven runtime Lua files, four icon assets, twelve datasets, and player documentation/metadata. Build verification rejects diagnostics, probes, debug files, and saved settings, and checks that all runtime modules are included.
...
Eight Lua test files cover full-map labels and symbols, settings isolation and validation, diagnostics, nearby selection, minimap slot restoration, fog visibility and recovery, atlas switching, and sprite expansion/reuse/failure handling. Fog regressions exercise the actual main-script controls for all three groups, all eight combinations of preferences, saved preferences, acquisition rules, exploration with cached markers, local-area changes, unavailable data, and filtering before the minimap limit. The minimap stress fixture renders 200 custom markers alongside 56 native icons. Runtime Lua files are syntax checked. Asset validation checks that every original atlas entry is unchanged and the custom texture and UV coordinates are valid.
