...
(async () => {
  for (const [output, names] of [
    ['markers', ['token', 'beetle', 'chest', 'special-chest']],
    ['height-arrows', ['height-up', 'height-down']],
  ]) {
    ...
    await sharp({ create: { width: names.length * 128, height: 128, channels: 4,
      background: { r: 0, g: 0, b: 0, alpha: 0 } } })
      .composite(tiles).png().toFile(path.join(root, `${output}.png`));
  }
